...
public class Model {
	...
	private static final int[][] DIS = {{0, 1}, {1, 0}, {0, -1}, {-1, 0}};

	private boolean tileEquals(Tile t1, Tile t2) {
		if (t1 == t2) {
			return true;
		}
		if (t1 == null || t2 == null) {
		...
		for (int i = 0; i < size(); i++) {
			for (int j = 0; j < size(); j++) {
				...
				for (int[] di : DIS) {
					...
					Tile nextTile = tile(x, y);
					if (tileEquals(nextTile, tile)) {
						return true;
					}
				}
			}
	...
	public void moveTileUpAsFarAsPossible(int x, int y) {
		...
		for (int i = y + 1; i < size(); i++) {
			Tile tile = tile(x, i);
			if (tile != null) {
				if (tileEquals(tile, currTile) && !tile.wasMerged()) {
					++targetY;
				}
				break;
			}
